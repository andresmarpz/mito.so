...
const features = [
  ...
  },
  {
    ...
    description:
      "Create collections to categorize your bookmarks however you want.",
  },
  {
    icon: FileSearchIcon,
  ...
  },
  {
    ...
    description:
      "Navigate, search, and manage everything without touching your mouse.",
  },
  {
    icon: LockIcon,
    title: "Private by default",
    description:
      "Your bookmarks are yours alone. No ads, no tracking, no data selling.",
  },
  {
    icon: LayoutIcon,
    title: "Minimal interface",
    description:
      "No clutter, no distractions. Just your bookmarks, clean and simple.",
  },
  {
    icon: MonitorIcon,
    title: "Access anywhere",
    description:
      "Web-based means no apps to install. Works on any device with a browser.",
  },
];
      ...
      <main className="flex-1 max-w-md m-auto px-6 py-12 sm:py-24">
        ...
        <div className="flex flex-col items-center text-center gap-5">
          ...
          <div className="flex items-center gap-2 pt-1">
            <Button className="rounded-full px-5" nativeButton={false} render={<Link to="/sign-in" />}>
              Sign up
            </Button>
            <Button variant="outline" className="rounded-full px-5" nativeButton={false} render={<Link to="/sign-in" />}>
              Log in
            </Button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-5 sm:gap-7 pt-16 sm:pt-24">
          {features.map((feature) => (
            <div key={feature.title} className="flex items-start gap-4">
              <feature.icon
                className="size-5 text-zinc-700 shrink-0 mt-0.5"
                strokeWidth={1.5}
              />
              <div>
                <h2 className="font-medium text-zinc-900 text-sm sm:text-base">
                  {feature.title}
                </h2>
